...
print "Loaded all chr10 files into memory\n";
...
if ($temp[11] == 0)  {
		...
		@temp = split('\t',$lines[$j]);
			...
			if ( ($j == ($k - 1) ) and ($temp[11] == 0) )  {
			print "entered this loop: j is $j and K is $k\n";
			print "line is $lines[$j]\n";
			$start = $temp[3];
			$stop = 247190999;
			if ( ($stop - $start < $size) & (abs($temp[10]) > $LR) ) {
			print ACCEPT_BREAKPOINT "$name\t$temp[4]\t$start\t$stop\t99\ttelomere";
				}
			else {
			print REJECT_BREAKPOINT "$sample\t$temp[4]\t$start\t$stop\t99\ttelomere";
					}
					}
		...
		if ($temp[11] != 0)  {
		if ( ($stop - $start < $size) & (abs($temp[10]) > $LR) ) {
		print ACCEPT_BREAKPOINT "$name\t$temp[4]\t$start\t$stop\t$temp[10]\t$temp[11]";
				}
		else {
		print REJECT_BREAKPOINT "$sample\t$temp[4]\t$start\t$stop\t$temp[10]\t$temp[11]";
				}
				}
		$start = $stop;
...
if ($temp[11] != 0)  {
	...
	@temp = split('\t',$lines[$j]);
		...
		$stop = $temp[3];
		if ($temp[11] != 0)  {
		if ( ($stop - $start < $size) and (abs($temp[10]) > $LR) ) {
		print ACCEPT_BREAKPOINT "$name\t$temp[4]\t$start\t$stop\t$temp[10]\t$temp[11]";
				}
		else {
		print REJECT_BREAKPOINT "$sample\t$temp[4]\t$start\t$stop\t$temp[10]\t$temp[11]";
...
open (CHRX, "$sample.chrX_breakpoint_.txt");
@lines = <CHRX>;
$k = @lines;
$j = 1;
@temp = split('\t',$lines[$j]);
...
if ($temp[11] != 0)  {
	...
	#print BREAKPOINT "$sample\t$temp[4]\t$start\t$stop\t$temp[10]\t$temp[11]";
	while ($j < $k)  {
	$start = $stop;
	$j = $j + 1;
	@temp = split('\t',$lines[$j]);
...
open (CHRY, "$sample.chrY_breakpoint.txt");
@lines = <CHRY>;
$k = @lines;
$j = 1;
@temp = split('\t',$lines[$j]);
