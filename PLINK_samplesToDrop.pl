...
# 1. Input data: List of sample IDs - *.sampleIDs.txt
...
# Remove old files
unlink("*.samplesToDrop.txt");
unlink("log.samplesToDrop.txt");

# Open for output of log files
open(OUT_LOG, ">>", "log.samplesToDrop.txt")
	or die "Cannot open OUT_LOG for log output";

# Obtain samples names to drop
@drop_names = glob("*_sampleIDs.txt");
$drop_names_number = ($#drop_names + 1);
if ($drop_names_number == 0) {
	die "$drop_names_number sampleIDs files detected.\n
...
open (OUT_ANNOT, ">>", "PLINK_samplesToDrop.txt")
	or die "Cannot open OUT_ANNOT filehandle to PLINK_samplesToDrop file";

# Process *_sampleIDs.txt file
foreach my $name (@drop_names) {
    
    #Open $name to be read
    open (IN_NAMES, "<", "$name")
        or die "Cannot open IN_ANNOT filehandle to read *_sampleIDs.txt file";
...
foreach my $p (@plink_file) {
    ...
    while (<IN_PLINK>) {
        ...
        $pedindv{"$temp_line[1]"} = "$temp_line[0]";
    }
close (IN_PLINK);
}
...
foreach my $n (@temp_names) {
    ...
    print OUT_ANNOT "$pedindv{$n}\t$n\n";
}

print "Completed PLINK_samplesToDrop script\n";
print OUT_LOG "Completed PLINK_samplesToDrop script\n";
close (OUT_LOG);
