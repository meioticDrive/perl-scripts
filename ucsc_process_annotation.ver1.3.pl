...
my(@ucsc_file, $ucsc_file_number, @ucsc_labels, @ucsc_gene_info, @ucsc_exonStarts, @ucsc_exonEnds, $exon_number, $temp_start, $temp_end, $coding_start, $coding_end, $loop_count, $coding_exon_counter);
...
foreach my $files (@ucsc_file) {
	...
	# Subsequent lines are gene information
	while (<IN_ANNOT>) {
		chomp($_);
		...
		# Process genes with 2 exons, 1 coding fragment
		if (($ucsc_gene_info[6] == 2) && ($coding_start > $ucsc_exonEnds[0])) {
			print "Made it into special 2 message exon loop\n";
		...
		# Process Genes with more than 2 exons
		for (my $i = 0; $i < $exon_number; $i++) {
