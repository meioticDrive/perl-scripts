...
open (OUTFILE_0, ">", "Cutler_TDT_pvalue.txt");
...
while(<IN_DATA_IN>) {
    ...
    if($_ =~ /^SNP/) {
        @header = split (/\t/, $_);
        print OUTFILE_0 "$header[0]\t$header[1]\t$header[2]\t$header[4]\n";
        next;
	}
	
	# Get data from lines
	@data_line = split (/\t/, $_);
	print OUTFILE_0 "$data_line[0]\t$data_line[1]\t$data_line[2]\t$data_line[4]\n";
