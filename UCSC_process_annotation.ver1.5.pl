...
foreach my $files (@ucsc_file) {
	...
	while (<IN_ANNOT>) {
		...
		if ($ucsc_gene_info[6] == 1) {
			...
			print "Exiting the 1 exon, 1 coding fragment\n\n";
			next;
		}
...
close (OUT_CODING);
close (IN_ANNOT);
print OUT_LOG "Completed ucsc_process_annotation.ver1.5.pl program.\n";
close(OUT_LOG);
print "Completed ucsc_process_annotation.ver1.5.pl program.\n";
