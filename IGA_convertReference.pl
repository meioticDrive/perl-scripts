...
system ("rm *.convert.reference.txt");

# Collect the names of reference fasta file in a directory
@pileup_files = glob("*fasta.sort.fasta");

# Open output file for coverage values
open(OUT_IGA_PILEUP_STATS, ">", "IGA.convert.reference.txt")
...
print "Completed IGA_convertReference.pl version $version script\n";