#!/usr/bin/perl -w
# Version 1.0
# Copyright Michael E. Zwick, 2015_08-25
################################################################################
# Program designed to:
...
print OUT_ANNOT "Summary statement concerns that are likely to preclude funding\n\n";
parsetext ("combined.temp.txt", "Summary statement concerns", "Will the candidate's plan");

print OUT_ANNOT "Will the candidate's plan adequately address these concerns?\n\n";
parsetext ("combined.temp.txt", "Will the candidate's plan", "From the summary statement");
...
sub facultyname {
	...
	foreach my $files (@text_file) {
		...
		while (<IN_ANNOT>) {
			if (($_ =~ /^Faculty Candidate/) && ($count ==0)) {
				@line = split (/:/, $_);
				$line[1] =~ s/^\s+|\s+$//g;
