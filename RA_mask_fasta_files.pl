...
$fragment_start, $fragment_end, $uniquemer_positions_size, $version);
...
# Glob all file names
@all_fasta_files = glob("*.edit.fasta")
	or die "Did not find any files ending with *.edit.fasta\n";
...
for $file_name (@all_fasta_files) {
    ...
    # files
    open(OUT_FASTA_FILES, ">", "$file_name.30mer.mask.fasta")
...
print "Completed RA_mask_fasta_files.pl script version $version\n";
