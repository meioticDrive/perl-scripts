...
# 05/13/2014
################################################################################
# PLINK_parseTDT_forDatagraph.pl
# Program designed to generate and format PLINK tdt output data from PLINK for easy 
# entry into DataGraph
# 1. Parse SNP IDs and p-values from plink tdt file
# 2. Determines start/stop bounds for each chromosomes
#
# Future Updates
# 1. Capture file name from test before *.tdt file
...
open(OUT_LOG, ">>", "log.tdt.parsed.txt")
...
print "Detected $plink_file_number *.tdt.txt file\n";
print OUT_LOG "Detected $plink_file_number *.tdt file\n";

# Open output file for complete tdt data
open (OUT_ANNOT, ">>", "tdt.parsed.txt")
...
foreach my $files (@plink_file) {
    ...
    while (<IN_ANNOT>) {
        chomp $_;
        @temp_line = split(/\s+/, $_);
        if ($temp_line[1] eq "CHR") { next;} # Skip header line
        $logPscore = (-1 * log10($temp_line[12])); # Calculate -1Log10P
        print OUT_ANNOT "$count\t$temp_line[1]\t$temp_line[2]\t$temp_line[3]\t$temp_line[4]\t$temp_line[5]\t$temp_line[6]\t$temp_line[7]\t$temp_line[8]\t$temp_line[9]\t$temp_line[10]\t$temp_line[11]\t$temp_line[12]\t$logPscore\n";
        print OUT_COUNT "$count\n";
        print OUT_COUNTCHR "$count\t$temp_line[1]\n";
        print OUT_PVALUE "$temp_line[12]\n";
