...
# fasta files generated by a chip resequencing experiment
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
use strict;
...
#-------------------------------------------------------------------------------
@reference_chip_files = glob("*.ames.reference.chip");
$ref_chip_file_number = ($#reference_chip_file + 1);

print "Processing a total of $ref_chip_file_number reference files\n";
if ($ref_chip_file_number == 0) {
	die "$ref_chip_file_number .fasta files detected.\n
...
#-------------------------------------------------------------------------------
@reference_shotgun_file = glob("*.reference.shotgun");
$ref_shotgun_file_number = ($#reference_shotgun_files + 1);

print "Processing a total of $ref_shotgun_file_number reference files\n";
if ($ref_shotgun_file_number == 0) {
	die "$ref_shotgun_file_number .fasta files detected.\n
		Check directory. Exiting program";
}
...
#-------------------------------------------------------------------------------
foreach my $ref_file (@reference_shotgun_file) {
	...
	print OUT_DISCREPANCIES "Size of reference sequence: $reference_sequence_size\n";
}
...
foreach my $exp_files (@experimental_files) {
	...
	for (my $j = 0; $j < $reference_sequence_size; $j++) {
		...
		if (($experiment_compare[$j] eq "N") || ($ref_compare[$j] eq "N")) {
			...
			print OUT_DISCREPANCIES "Reference sequence base is $ref_compare[$j]\n";
			print OUT_DISCREPANCIES "20bp Upstream: ";
			for (my $i = 20; $i > 0; $i--) {
				print OUT_DISCREPANCIES "$ref_compare[$j-$i]";
			}
			print OUT_DISCREPANCIES "\n";
			print OUT_DISCREPANCIES "20bp Downstream: ";
			for (my $k = 1; $k < 21; $k++) {
			...
			print OUT_DISCREPANCIES "20bp Upstream: ";
			for (my $l = 20; $l > 0; $l--) {
				print OUT_DISCREPANCIES "$ref_compare[$j-$l]";
			}
			print OUT_DISCREPANCIES "\n";
			print OUT_DISCREPANCIES "20bp Downstream: ";
			for (my $m = 1; $m < 21; $m++) {
				print OUT_DISCREPANCIES "$ref_compare[$j+$m]";
			}
			print OUT_DISCREPANCIES "\n";
			$bases_different++;
			$j++;
