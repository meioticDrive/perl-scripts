...
# 06/28/05
# Program designed to:
# 1. Run over multiple folders (multi) containing fasta sequences and annotation 
# files (.genpos.txt, .msg.txt, .coding.txt).
# 2. Build an input file (in.pop.in) in each folder in the directory, then call 
# Dave Cutler's popgen_fasta C program to calculate all popgen parameters.
# 3. This code is for datasets with diploid sequences.
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
use strict;
use Cwd;
...
open(OUT_LOG, ">", "log.diploid.popgen_fasta.out")
...
close(OUT_LOG);
print "Completed run_multi_popgen_ver1.0.pl program.\n";
