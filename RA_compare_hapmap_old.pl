...
my ($dirname, $version, $temp_seq, @temp_label, $count, @fasta_files,
@label_genpos, @hapmap_files, @temp_genpos, @genpos_name, @genpos_start,
@genpos_end, @temp_hapmap, $nmbr_genpos_name, $nmbr_genpos_start,
$nmbr_genpos_end, @temp_ra_seq, $temp_location, @files, @filess,
@fasta_labels, @temp_seq, %fasta_info, $file_name, $i);
...
# Individual RA sequence
@fasta_files = glob("*.fasta") 
    or die "No fasta file found\n";
    
# Fasta labels genpos of reference sequences
...
foreach my $process_file (@fasta_files) {
  ...
  open(FILEHANDLE_FIRST, $process_file)
	...
	while (<FILEHANDLE_FIRST>) {
		...
		if($_ =~ />/) {
			if($count > 0) {
				# Make hash relating fasta label with 
				#sequence (from previous loop through)
				#print "$temp_seq\n";
				#print "$temp_label[0]\n";
